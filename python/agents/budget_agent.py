...
def compute_breakdown(state: TravelPlanState) -> BudgetBreakdown:
    """由代码计算预算明细 —— ReplanAgent 的 LLM 循环也复用此函数,
    保证"算术永远归代码、不信 LLM 的算术"。"""
    ...
    remaining = state.preferences.budget - total
    return BudgetBreakdown(
        ...
        over_budget_amount=max(0, -remaining),
    )
...
class BudgetAgent(BaseAgent):
    ...
    def __init__(self, auto_adjust: bool = True) -> None:
        """auto_adjust=False 时只做校验与状态判定, 不执行规则砍价
        (REPLAN_MODE=agent 下调整动作由 ReplanAgent 的 LLM 决策)。"""
        super().__init__()
    ...
    async def execute(self, state: TravelPlanState) -> TravelPlanState:
        ...
        if not breakdown.is_within_budget:
            breakdown.suggestions = self._generate_suggestions(
                breakdown.over_budget_amount,
        ...
        over_amount = breakdown.over_budget_amount

        if breakdown.is_within_budget:
            state.state = PlanningState.COMPLETED
            logger.info(f"[{self.name}] 预算通过! 总费用 ¥{total:.0f}, 剩余 ¥{breakdown.remaining:.0f}")
            ...
            if self.auto_adjust:
                ...
                logger.warning(f"[{self.name}] 超预算 ¥{over_amount:.0f}, 待 ReplanAgent 处理")
        else:
            state.state = PlanningState.COMPLETED
